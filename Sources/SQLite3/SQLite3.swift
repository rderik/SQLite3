...
public class CSQLite3 {
  ...
  public func executeQuery(_ query: String) -> [[String]] {
    var query = "select name from Elements"
    var result = [[String]]()
    ...
    if(sqlite3_prepare_v2(db, query, -1, &selectStatement, nil) == SQLITE_OK) {
      ...
      let columnNames: [String] = (0..<Int32(self.columnCount)).map {
      ...
      while(sqlite3_step(selectStatement) == SQLITE_ROW) {
        ...
        var element = String(cString: sqlite3_column_text(selectStatement,i))
        row.append(element) 
        }
        result.append(row)
      }
